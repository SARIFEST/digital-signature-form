...
// const pdfController = require('./pdfRout');
...
//       if (err) return res.status(500).send('שגיאה בהמרה');
//       fs.writeFileSync(outputPath, done);
//       res.sendFile(outputPath);
//     });
//   } catch (error) {
//     res.status(500).send('שגיאה בצד השרת');
//   }
// });

// //  המרת והצגת קובץ PDF באמצעות שירות
// router.get('/form/view/:fileName', formService.viewForm);

// //  קבלת חתימה, המרת Word ל־PDF, שילוב חתימה, ושליחת מיילים
...
//       if (err) return res.status(500).send('שגיאה בהמרת הקובץ');
...
// // סימון טופס כחתום ושליחה במייל
// router.post('/form/approve-signature', async (req, res) => {
//   const { id } = req.body;
//   const form = formRepository.getFormById(id);
//   if (!form) return res.status(404).json({ error: 'Form not found' });

//   form.signed = true;
//   formRepository.saveForms(formRepository.getForms());
...
//     await formRepository.sendEmailWithFile(form.email, pdfPath);
//     return res.json({ message: 'נחתם ונשלח במייל' });
//   });
// });
...
// //  המרת מסמך PDF לפי מזהה טופס
// router.get('/pdf/:id', pdfController.convertToPdf);

// //  שליחת טופס עם מייל ולינק לחתימה
...
// //  יצירת טופס ושמירת נתוני משתמש
// router.post('/upload-form-data', async (req, res) => {
//   try {
//     const { name, email, fileName } = req.body;
//     if (!name || !email || !fileName) return res.status(400).json({ error: 'Missing data' });
//     const form = await formService.uploadForm(name, email, fileName);
...
// // הצגת טופס לפי מזהה דרך נתיב מלא
